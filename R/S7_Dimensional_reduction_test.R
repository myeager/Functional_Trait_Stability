...
#uni.spc <- colnames(spc.df[3:ncol(spc.df)])
#uni.spc <- gsub("[\\.]", " ", uni.spc)
...
beta.yr_to_cent_dist <- merge(avg.yr_centroid_dist, pal, by = "Pond")
...
##Pond
###Test
summary(aov.mod <- aov(mean_rel_distance ~ Pond, beta.yr_to_cent_dist))
TukeyHSD(aov.mod)
###Plot
boxplot(beta.yr_to_cent_dist$mean_rel_distance ~ Pond, beta.yr_to_cent_dist, col = pal$col, ylab = "Relative year to centroid distance")
points(aggregate(mean_rel_distance ~ Pond, data = beta.yr_to_cent_dist, mean)[2], pch = 16)
##Type
###Test
summary(aov.mod <- aov(mean_rel_distance ~ type, beta.yr_to_cent_dist))
TukeyHSD(aov.mod)
###Plot
boxplot(beta.yr_to_cent_dist$mean_rel_distance ~ type, beta.yr_to_cent_dist,
        col = c("#506E82", "#768655"), ylab = "Relative year to centroid distance",
        names = c("Species", "Functional Traits"), xlab = "")
points(aggregate(mean_rel_distance ~ type, data = beta.yr_to_cent_dist, mean)[2], pch = 16)


#yr to yr
avg.yr_yr_dist_FT <- MCMC_yr_yr_dist_FT %>%
  group_by(Pond, Year) %>%
  summarise(mean_rel_distance = mean(rel_distance), mean_distance = mean(distance))%>%
  mutate(type = "traits")


avg.yr_yr_dist_COM <- MCMC_yr_yr_dist_COM %>%
  group_by(Pond, Year) %>%
  summarise(mean_rel_distance = mean(rel_distance), mean_distance = mean(distance))%>%
  mutate(type = "species")
...
beta.yr_to_yr_dist <- merge(avg.yr_yr_dist, pal, by = "Pond")
summary(aov(mean_rel_distance ~ Pond * type, beta.yr_to_yr_dist))
summary(aov(mean_rel_distance ~ Pond, beta.yr_to_yr_dist))
summary(aov(mean_rel_distance ~ type, beta.yr_to_yr_dist))
mod <- aov(mean_rel_distance ~ Pond, beta.yr_to_yr_dist)
TukeyHSD(mod)
##Pond
###Test
summary(aov(mean_rel_distance ~ Pond, beta.yr_to_yr_dist))
###Plot
boxplot(beta.yr_to_yr_dist$mean_rel_distance ~ Pond, beta.yr_to_yr_dist, col = pal$col, ylab = "Relative year to year distance")
points(aggregate(mean_rel_distance ~ Pond, data = beta.yr_to_yr_dist, mean)[2], pch = 16)

###Test
summary(aov.mod <- aov(mean_rel_distance ~ type, beta.yr_to_yr_dist))
TukeyHSD(aov.mod)
yr.yr.ty.pval <- round(summary(aov.mod)[[1]][["Pr(>F)"]][1],3)
boxplot(beta.yr_to_yr_dist$mean_rel_distance ~ type, beta.yr_to_yr_dist,
        col = c("#506E82", "#768655"), ylab = "Relative year to year distance",
        names = c("Species", "Functional Traits"), xlab = "")
points(aggregate(mean_rel_distance ~ type, data = beta.yr_to_yr_dist, mean)[2], pch = 16)
...
ggplot(beta.yr_to_yr_dist, aes(x=factor(Pond),y=mean_rel_distance,fill=factor(type)))+
