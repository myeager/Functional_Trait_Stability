...
TL <- read.csv("/Users/mallarie.yeager/Documents/R_projects/FT Stability/SIMPER analysis/Data/fish_diets.csv")
#set color pal
wes_palettes <- list(
...
#Plot Simper 
```{r}
# co <- as.data.frame(colorRampPalette(brewer.pal(9,"BrBG"))(40))
# co <- as.data.frame(co[c(1:7,13:23,22:37),])
# colnames(co)[1] <- "col"
# co$num <- 1:27
# co <- co[order(co$num, decreasing = T),]
#co$Species <- spcs$Common_name
...
```{r ENV FIT BIPLOT}
# spcs <- colnames(sums[,3:ncol(sums)])
# # forage <- as.data.frame(spcs)
# # forage$forage <- "non forage"
# # forage[c(1:2,4,6,13,17,19),2] <- "forage"
# # forage$col[forage$forage == "non forage"] <- "#A16407"
# # forage$col[forage$forage == "forage"] <- "#1A847C"
# #TL <- read.csv("fish_diets.csv")
# #TL <- merge(TL, spcs,  by = "Common_name")
# 
#  TL <-TL %>%
#    arrange(factor(Feeding_habit, levels = c("detritovore", "plankton feeding", "hunting meiofauna", "hunting macrofauna")))
#   TL$col <- c(col.detrit, col.plankt, col.meio, col.macro)
# 
# #TL <- merge(TL, forage, by.x = "Common_name",by.y = "spcs")
# #TL$Troph <- ifelse(TL$forage == "forage", "Forage Fish", TL$Trophic_group)
# fish.sums <- aggregate(FREQ ~ Common_name + Pond + Year, data = comm.df, sum) #sums freq up for all spc * size combos
...
set.seed(12)
#(FT.fit <- envfit(com.mds, FT.sums[3:ncol(FT.sums)]))
